...
<html>
    ...
    <body>
        <h1 style="margin-left: 37%">Cadastro de Cenário</h1>
         ...
         <input type="hidden" name="idcenario" id="idcenario" value="<?php echo $_POST['idcenario']?>">
         <div style="margin-left:40%;">
             <label>Sistema</label><br/>
             <input type="text" name="nomesistema" value="<?php echo $_POST['nomesistema']?>" readonly="readonly">
			...
			<input type="text" name="nomesubsistema" value="<?php echo $_POST['nomesubsistema']?>" readonly="readonly">
			<!--<select name="idsubsistema" id="idsubsistema" >
			</select>--><br/>
            <label>Nome do Caso de Uso</label><br/>
            <input type="text" name="nomecasoUso" value="<?php echo $_POST['nomecasoUso']?>" readonly="readonly">
            <!--<select name="idcasoUso" id="idcasoUso">
            </select>-->
			</div>
            <table style="margin-left: 20%;">
                <?php include "../frm/ManagerCenario.php";
                	$cenario = new BdCenario();
                	$retorno = $cenario->listar($_POST['idcenario']);
                	
                ?>
                <tr>
                    <td >Breve Descritivo:</td>
                    <td ><textarea name="descricao" cols="50" rows="2" id="descricao"><?php echo $retorno[0]->descricao?></textarea></td>
                ...
                <?php $i = 0; 
                	foreach ($retorno as $passo){
                	if($valores->id == $passo->idfluxo){
                		echo "<tr><td>P".$passo->nrPasso."<input type='hidden' value='".$passo->nrPasso."' name='passo".$passo->nrPasso."'</td>";echo "<td><textarea name='passoDesc".$valores->id."+".$passo->nrPasso."' cols='50' rows='2' id='passo".$passo->nrPasso."'>".$passo->descrPasso."</textarea></td></tr>";
                		$i++;
                		?><input type="hidden" name="passo<?php echo $valores->id?>" value="<?php echo $i?>" id="theValue<?php echo $valores->id?>"><?php
                	}else{
                		$i = 0;
                	}
                }?>
                ...
                <input type="submit" value="Editar" id="editar" name="editar" style="margin-left: 40% ;">
         </form>
    </body>
